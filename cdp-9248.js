...
function getJSON(p){return new Promise((res,rej)=>{http.get('http://127.0.0.1:9248'+p,r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>{try{res(JSON.parse(d))}catch(e){rej(e)}});}).on('error',rej);});}
